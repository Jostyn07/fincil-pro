...
export default function Inventario() {
  ...
  const [materiaEditando, setMateriaEditando] = useState<MateriaPrima | null>(null)
  ...
  function limpiarFormProducto() {
    ...
    setCostoManual(false); setProductoEditando(null); setMostrarFormProducto(false)
  }

  function limpiarFormMateria() {
    setMNombre(''); setMUnidad(''); setMStock(0)
    setMStockMin(0); setMCosto(0); setMProveedor(''); setMObservaciones('')
    setMateriaEditando(null); setMostrarFormMateria(false)
  }
  ...
  function editarProducto(p: Producto) {
    ...
    setPTipo(p.tipo)
    setPCosto(p.costo_unitario)
    setPPrecio(p.precio_venta)
    setPStockInicial(p.stock_disponible)
    setPStockReservado(p.stock_reservado)
    setPStockMin(p.stock_minimo)
    setPObservaciones(p.observaciones)
    setCostoManual(true)
    setMostrarFormProducto(true)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }

  function editarMateria(m: MateriaPrima) {
    setMateriaEditando(m)
    setMNombre(m.nombre)
    setMUnidad(m.unidad)
    setMStock(m.stock_actual)
    setMStockMin(m.stock_minimo)
    setMCosto(m.costo_unitario)
    setMProveedor(m.proveedor)
    setMObservaciones(m.observaciones)
      ...
      {pestana === 'productos' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => { limpiarFormProducto(); setMostrarFormProducto(true) }}
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={pTipo} onChange={(e) => { setPTipo(e.target.value); setCostoManual(false) }}
                  ...
                  {pTipo === 'Producido' && !costoManual && (
                    <div className="mt-2 space-y-1">
                      <p className="text-xs text-teal-600">
                        💡 Al guardar podrás calcular el costo en <strong>Recetas</strong>.
                      </p>
                      <button type="button" onClick={() => setCostoManual(true)}
              ...
              <div className="flex gap-3 mt-4 justify-end">
                <button onClick={limpiarFormProducto}
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500">ACCIONES</th>
                  </tr>
                </thead>
                <tbody>
                  {cargando ? (
                    <tr><td colSpan={11} className="py-12 text-center text-gray-400">Cargando...</td></tr>
                  ) : productos.length === 0 ? (
      ...
      {pestana === 'materias' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => { limpiarFormMateria(); setMostrarFormMateria(true) }}
              className="bg-[#0f1e35] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#162740] transition-colors">
          ...
          {mostrarFormMateria && (
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-4">
              <h3 className="font-semibold text-gray-800 mb-4">
                {materiaEditando ? 'Editar materia prima' : 'Nueva materia prima'}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex gap-3 mt-4 justify-end">
                <button onClick={limpiarFormMateria}
                  className="border border-gray-200 text-gray-600 px-4 py-2 rounded-lg text-sm hover:bg-gray-50">
                  Cancelar
                </button>
                    ...
                    materias.map((m) => {
                      ...
                      return (
                        <tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="py-3 px-3">
                            <button onClick={() => editarMateria(m)}
                              className="text-xs text-[#00c9a7] hover:underline font-medium">
                              Editar
                            </button>
                          </td>
